...
public class Helper {
    public static String chomp(String inStr) {  
        ...
        if (lastChar == 13) { return inStr.substring(0, inStr.length() - 1); }
        else if (lastChar == 10) {
    ...
    public static String[] splitString(String str, char sep, int maxNum) {
            ...
            if (c == sep) {
                if (maxNum != 0) {
                    if ((results.size() + 1) == maxNum) {
                        for (; i < str.length(); i++) buf.append(str.charAt(i));  
                        break;  
                    }
                }
        ...
        if (buf.length() > 0) results.addElement(buf.toString());
